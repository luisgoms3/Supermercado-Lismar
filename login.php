...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty(trim($_POST["email"]))){
        $email_err = "Porfavor introduza o email.";
    } else{
    ...
    if(empty(trim($_POST["password"]))){
        $password_err = "Porfavor introduza a palavra-passe.";
    } else{
    ...
    if(empty($email_err) && empty($password_err)){
        ...
        $sql = "SELECT email, nome, palPasse FROM utilizadores WHERE email = ?";
        ...
        if($stmt = mysqli_prepare($mysqli, $sql)){
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                if(mysqli_stmt_num_rows($stmt) == 1){                    
                    ...
                    mysqli_stmt_bind_result($stmt, $email, $nome, $md5_password);
                    if(mysqli_stmt_fetch($stmt)){
                        if(md5($password) == $md5_password){
                            ...
                            $password_err = "A palavra-passe esta errada.";                            
                        }
                    }
                } else{
                    // Display an error message if username doesn't exist
                    $email_err = "Esse email não existe.";
                }
            } else{
                echo "Oops! Algo correu mal. Porfavor tenta mais tarde.";
            }
...
<html style="font-size: 16px;">
  ...
  <body class="u-body"><header class="u-clearfix u-header u-header" id="sec-ef04"><div class="u-clearfix u-sheet u-sheet-1">
    ...
    <section class="u-clearfix u-grey-5 u-section-1" id="sec-47f6">
      <div class="u-clearfix u-sheet u-sheet-1">
        <a href="criarconta.php" data-page-id="275700447" class="u-btn u-btn-round u-button-style u-hover-palette-1-dark-1 u-palette-1-base u-radius-10 u-btn-1">Criar conta</a>
        ...
        <h3 class="u-text u-text-palette-1-base u-text-1">Iniciar Sessão</h3>
        <h5 class="u-text u-text-2">Ainda não tens conta ?</h5>
        <p class="u-small-text u-text u-text-variant u-text-3">Se ainda não tens conta cria aqui.</p>
        <div class="u-form u-form-1">
          <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="POST" class="u-clearfix u-form-spacing-10 u-inner-form" class="" style="padding: 10px" source="custom" name="form">
            <div class="u-form-email u-form-group <?php echo (!empty($email_err)) ? 'has-error' : ''; ?>">
              <label for="email-fbef" class="u-form-control-hidden u-label u-label-1">Email</label>
              <input type="email" placeholder="Insira um endereço de email válido" id="email-fbef" name="email" class="u-border-1 u-border-grey-30 u-input u-input-rectangle u-radius-10 u-white" value="<?php echo $email; ?>" required="">
              <span>&nbsp;<?php echo $email_err; ?></span>
            </div>
            <div class="u-form-group u-form-name <?php echo (!empty($password_err)) ? 'has-error' : ''; ?>">
              <label for="name-fbef" class="u-form-control-hidden u-label u-label-2">Nome</label>
              <input type="password" placeholder="**************" id="name-fbef" name="password" class="u-border-1 u-border-grey-30 u-input u-input-rectangle u-radius-10 u-white" required="">
              <span><?php echo $password_err; ?></span>
            </div>
            <div class="u-align-center u-form-group u-form-submit">
              <a href="#" class="u-btn u-btn-round u-btn-submit u-button-style u-radius-10">Iniciar Sess&atilde;o<br></a>
